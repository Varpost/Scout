...
Honest reading of the [v0.1.8 results](benchmarks/results/0.1.8/summary-native.md): command-injection detection is the strongest category (22% precision / 19% recall against CVEs that full taint-analysis engines also find hard); the XSS heuristics are deliberately narrow and noisy on legacy codebases; JS SQL injection through ORM APIs is out of pattern reach today. The false-positive counts are an upper bound by benchmark convention — unlabeled real issues count against Scout. These numbers are published to invite fair comparison and to be improved release over release, not to impress.
